...
```sh
git clone https://github.com/<owner>/ida-break.git
ln -s "$(pwd)/ida-break" ~/.idapro/plugins/ida-break
```

IDA를 재시작하면 `ida_break_entry.py`가 `PLUGIN_ENTRY`로 잡혀 로드됨.

## 사용법

Pseudocode 뷰 (`F5`로 디컴파일된 창)에서:

- **시작/종료**: `Ctrl-Alt-K` 또는 우클릭 메뉴 *"ida-break: Start brick break"*
- **이동**: `←` / `→` (또는 `h`/`l`, `a`/`d`)
- **발사**: `Space`
- **재시작**: `R` (WIN/LOSE 화면에서)
- **종료**: `Esc`
...
```
ida_break_entry.py       # PLUGIN_ENTRY shim, should_load() 환경 게이트
ida_break.py             # plugin_t / plugmod_t, 액션, UI/Hex-Rays 훅
ida_break_lib/
  game.py                 # 순수 물리 (Qt 의존 없음, 단위 테스트 가능)
  pseudocode.py           # viewport 탐지, bg 색 샘플링, 픽셀 brick 검출
  overlay.py              # BreakOverlay(QWidget) — 페인트, 입력, 타이머
```
...
`break_plugin_t(PLUGIN_MULTI)` → `break_plugmod_t`. plugmod에서:
...
`BreakOverlay`는 viewport의 자식 QWidget. `WA_TranslucentBackground`로 텍스트가
...
`ida_break_lib.pseudocode` 로거가 INFO 레벨로 다음을 출력 (IDA 기본 logging
레벨에서 출력창에 보임. 조용히 하고 싶으면 `logging.getLogger("ida_break_lib").
